...
export class XmlBuilder {
    ...
    buildXml(profileData: any, rules: XmlParserRuleOverride[], rootElement: string = ""): string {
        this.document = document.implementation.createDocument("", "", null);
        ...
        group.appendChild(this.createSetting("Xml_NS_NamespaceUse", "IgnoreNamespaces"));
        ...
        group.appendChild(this.createSetting("FileTypeConfiguration_ComponentBuilderId", "XML v 2.0.0.0"));
        group.appendChild(this.buildProfileData(profileData));
        console.log(this.document);
        return new XMLSerializer().serializeToString(this.document);
    }

    private createRuleSettings(rule: XmlParserRuleOverride, index: number): HTMLElement[] {
        const settings = [];

        settings.push(this.createSetting(`Xml_Translate_Rule_${index}`, "True"))
        settings.push(this.createSetting(`Xml_Translate_Rule_${index}Context`, "True"))
        settings.push(this.createSetting(`Xml_Translate_Rule_${index}CtxMustUseDisplayName`, "True"))
        settings.push(this.createSetting(`Xml_Translate_Rule_${index}XPathSelector`, rule.xpathSelector))

        if (!isNil(rule.translate)) {
            const value = rule.translate == TranslateRule.Yes ? "yes" : "no";
            settings.push(this.createSetting(`Xml_Translate_Rule_${index}Translate`, value));
        }

        settings.push(this.createSetting(`Xml_WithinText_Rule_${index}`, "True"))
        settings.push(this.createSetting(`Xml_WithinText_Rule_${index}Context`, "True"))
        settings.push(this.createSetting(`Xml_WithinText_Rule_${index}CtxMustUseDisplayName`, "True"))
        settings.push(this.createSetting(`Xml_WithinText_Rule_${index}XPathSelector`, rule.xpathSelector))

        if (rule.isInline) {
            settings.push(this.createSetting(`Xml_WithinText_Rule_${index}WithinText`, "yes"));
        }
    ...
    private buildProfileData(profileData: any): HTMLElement {
        ...
        const fileTypeInformation = this.document.createElement("FileTypeInformation");
        fileTypeInformation.setAttribute("xmlns", "http://www.sdl.com/filetypesupport");
        ...
        const fileTypeDocumentName = this.document.createElement("FileTypeDocumentName");
        fileTypeDocumentName.innerHTML = "XML Template Document";

        const fileTypeDocumentsName = this.document.createElement("FileTypeDocumentsName");
